...
import { getInvertedWeak, hexToRgba } from './themes/utils';
...
class Node extends React.Component<PropsType, StateType> {
  _head?: HTMLElement | undefined | null;
  _tail?: HTMLElement | undefined | null;
  _ownerWindow: any;
  ...
  render() {
    ...
    if (node.get('nodeType') === 'Wrapper') {
      return <span>{children.map(child => <WrappedNode key={child} id={child} depth={depth} />)}</span>;
    }
    ...
    if (nodeType === 'Text' || nodeType === 'Empty') {
      ...
      if (nodeType === 'Text') {
        ...
        tag = <span style={tagTextStyle(inverted, theme)}>"{text}"</span>;
          ...
          <div style={sharedHeadStyle} ref={h => (this._head = h)} {...headEvents}>
            <span>
              <span>
                ...
                {node.get('key') && <Props key="key" props={{ key: node.get('key') }} inverted={inverted} />}
                {node.get('ref') && <Props key="ref" props={{ ref: node.get('ref') }} inverted={inverted} />}
                {node.get('props') && <Props key="props" props={node.get('props')} inverted={inverted} />}
    ...
    const collapser = (
      <span
        title={hasState ? 'This component is stateful.' : ''}
    ...
    const head = (
      <div ref={h => (this._head = h)} style={sharedHeadStyle} {...headEvents}>
        ...
        <span>
          ...
          {node.get('key') && <Props key="key" props={{ key: node.get('key') }} inverted={headInverted} />}
          {node.get('ref') && <Props key="ref" props={{ ref: node.get('ref') }} inverted={headInverted} />}
          {node.get('props') && <Props key="props" props={node.get('props')} inverted={headInverted} />}
          <span style={sharedHeadBracketStyle}>&gt;</span>
          {selected && !collapsed && !this.props.isBottomTagSelected && <span style={jsxOpenTagStyle}> == $r</span>}
        </span>
        {collapsed && <span>…</span>}
        {collapsed && closeTag}
      </div>
    ...
    if (collapsed) {
      return <div style={styles.container}>{head}</div>;
    }
    ...
    return (
      <div style={styles.container}>
        ...
        <div>{children.map(id => <WrappedNode key={id} depth={depth + 1} id={id} />)}</div>
...
var WrappedNode = decorate(
  {
    listeners(props: PropsType) {
      return [props.id];
    },
    props(store: Store, props: PropsType) {
    ...
    shouldUpdate(nextProps: PropsType, prevProps: PropsType) {
...
const headStyle = ({
  ...
  isWindowFocused,
  theme
}: headStyleParams): React.CSSProperties => {
  let backgroundColor;
...
const tagTextStyle = (inverted: boolean, theme: Theme): React.CSSProperties => ({
  flex: 1,
  whiteSpace: 'nowrap',
  color: inverted ? getInvertedWeak(theme.state02) : theme.special06
});

const collapserStyle = (depth: number): React.CSSProperties => ({
  position: 'absolute',
  padding: 2,
