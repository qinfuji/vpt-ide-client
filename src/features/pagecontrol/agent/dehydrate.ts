 ...
 */
function dehydrate(data: any, cleaned: string[][], path?: string[] = [], level?: number = 0): string | Object {
  ...
  switch (type) {
    ...
    case 'object':
      if (
        level > 2 ||
        (data.constructor && typeof data.constructor === 'function' && data.constructor.name !== 'Object')
      ) {
        return createDehydrated(type, data, cleaned, path);
      } else {
        var res = {};
